...
public class MapImageServlet extends HttpServlet  {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        try {
            ...
            Path imagePath = Paths.get(String.format("%d_%d.png", id, mapImageSize));
            Path imageRepoPath = getImageRepoPath(imagePath);

            if (!Files.exists(imageRepoPath)) {
                List<MessageLocation> locations = messageService.getMessageLocations(id);
                ...
                if (locations.size() > 0) {
                    ...
                    image.flush();
                    log.info("Saved image for message " + id + " to file " + imageRepoPath);
                }
            }

            if (Files.exists(imageRepoPath)) {
                response.sendRedirect("/rest/repo/map_images/" + imagePath.toString());
            } else {
                response.sendRedirect("/img/map_image_placeholder.png");
            }
    ...
    /**
     * Computes the zoom level
     * @param bounds
     *
     * @return
     */
    private int computeZoomLevel(Point[] bounds, int maxWidth, int maxHeight, int maxZoomLevet, int minZoomLevel) {
        for (int zoom = maxZoomLevet; zoom > minZoomLevel; zoom--) {
    ...
    /**
     * Returns the repository path for the given message map image
     * @param imagePath the relative path of the message map image
     * @return the path
     */
    private synchronized Path getImageRepoPath(Path imagePath) throws IOException {
        Path mapImageFolder = repositoryService.getRepoRoot().resolve("map_images");
        if (!Files.exists(mapImageFolder)) {
            Files.createDirectories(mapImageFolder);
        }
        return mapImageFolder.resolve(imagePath);
    }
